...
export default function OrdersList({userOrders, loading, setPendingOrders, removeOrderFromPendingOrders}) {
    ...
    const acceptOrder = async(order_id) => {
        try {
            ...
            const responseData = await response.json();
            if(response.status === 200){
                console.log('The pending orders are:', userOrders)
                toast.success(responseData.message)
                removeOrderFromPendingOrders(order_id)
                // setPendingOrders(userOrders.filter(order => order.id !== order_id))
                // setPendingOrders(prevOrders => prevOrders.filter(order => order.id !== order_id));
            }else{
                toast.error(responseData.error)
            }
        } catch (error) {
